...
# -*- coding: utf-8 -*-
"""
多站点多时段风速垂直剖面网格可视化脚本（3行 x N列）。
用于展示 9 月 6–13 日各 LST 日的白天/晚上平均的三个站点的廓线变化。
"""
...
# ---------- 绘图核心函数 ----------
def plot_multi_day_avg_grid(df: pd.DataFrame, sites: list[str]) -> Path:
    days = cfg.metric_lst_days()
    periods = ["daytime", "nighttime"]
    
    col_configs = []
    for d in days:
        for p in periods:
            col_configs.append((d, p))
            
    # 设置一个更宽的画布以容纳10列
    fig, axes = plt.subplots(len(sites), len(col_configs), figsize=(50, 18), sharex=False, sharey=True)
    ...
    for row, site in enumerate(sites):
        for col, (day, period) in enumerate(col_configs):
            ax = axes[row, col]
            
            period_display = "Daytime" if period == "daytime" else "Nighttime"
            title = f"09-0{day} {period_display}"
            
            if row == 0:
                ax.set_title(title, fontsize=20, fontweight="bold", loc="left")
                
            lst_slots = _lst_slots_for(day, period)
            utc_labels = []
            for lst_ts in lst_slots:
    ...
    save_path = OUTPUT_DIR / "ws_grid_3x10_all_stations_avg.png"
...
def main() -> None:
    ...
    saved = plot_multi_day_avg_grid(df, list(DEFAULT_SITES))
    print(f"Saved: {saved}")
