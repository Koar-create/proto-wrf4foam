...
DATA_PATH = cfg.ANALYSIS_DIR / "metric_sample_variants_no_high_wide.csv"
OUTPUT_DIR = REPO_ROOT / "results/metric_sample_variants" / cfg.RESULTS_TAG
...
def daily_rows(df: pd.DataFrame) -> pd.DataFrame:
    out = df.loc[
        ...
        & (df["sample_date"] >= cfg.SAMPLE_DATE_MIN)
        & (df["sample_date"] <= cfg.SAMPLE_DATE_MAX)
    ].copy()
...
def build_figure(df: pd.DataFrame, spec: MetricSpec, line_mode: str = "delta") -> plt.Figure:
    ...
    fig.suptitle(
        f"{spec.title}, {cfg.CAMPAIGN_TITLE}",
        y=0.98,
        fontsize=13,
        fontweight="bold",
    )
