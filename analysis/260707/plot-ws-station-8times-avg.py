...
# -*- coding: utf-8 -*-
"""
多站点多时段风速垂直剖面网格可视化脚本（3行 x 4列）。
用于展示 9 月 6–13 日四个 synoptic UTC 时刻 (00/06/12/18，对应 LST 08/14/20/02) 的多日平均。
平均值通过 error bar 体现（参考 fig4 中的观测线）。
"""
...
OUTPUT_DIR = REPO_ROOT / "results/ws_wd_station_profile" / cfg.RESULTS_TAG / "multi_day_avg"
...
def _utc_slots_for_synoptic_avg(utc_hour: int) -> list[pd.Timestamp]:
    return [
        pd.Timestamp(f"2025-09-{day:02d} {utc_hour:02d}:00:00")
        for day in range(cfg.UTC_DAY_FIRST, cfg.UTC_DAY_LAST + 1)
    ]


def _lst_label_for_utc_hour(utc_hour: int) -> str:
    lst_hour = (utc_hour + 8) % 24
    return f"{lst_hour:02d}:00 LST ({utc_hour:02d}:00 UTC)"
...
def plot_multi_day_avg_grid(df: pd.DataFrame, sites: list[str], plot_diff: bool = False) -> Path:
    utc_hours = list(SYNOPTIC_UTC_HOURS)

    fig, axes = plt.subplots(len(sites), len(utc_hours), figsize=(20, 18), sharex=False, sharey=True)
    ...
    for row, site in enumerate(sites):
        for col, utc_hour in enumerate(utc_hours):
            ax = axes[row, col]

            title = _lst_label_for_utc_hour(utc_hour)

            if row == 0:
                ax.set_title(title, fontsize=24, fontweight="bold", loc="center")

            utc_slots = _utc_slots_for_synoptic_avg(utc_hour)
            utc_labels = []
    ...
    save_path = OUTPUT_DIR / f"ws_grid_3x4_synoptic_avg{suffix}.png"
