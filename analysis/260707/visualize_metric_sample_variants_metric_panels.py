...
def build_bar_panel_figure(df) -> plt.Figure:
    ...
    fig.suptitle(
        f"WRF vs. OpenFOAM metric summary bars, {cfg.CAMPAIGN_TITLE}",
        y=0.985,
        fontsize=13,
        fontweight="bold",
    )
...
def build_daily_panel_figure(df, line_mode: str = "delta") -> plt.Figure:
    ...
    suptitle = (
        f"Daily compact comparison and CFD improvement, {cfg.CAMPAIGN_TITLE}"
        if line_mode == "delta"
        else f"Daily WRF and OpenFOAM metric values, {cfg.CAMPAIGN_TITLE}"
    )
    fig.suptitle(
        suptitle,
        y=0.985,
