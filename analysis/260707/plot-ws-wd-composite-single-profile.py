...
OUTPUT_DIR = REPO_ROOT / "results/ws_wd_composite_profile" / cfg.RESULTS_TAG
...
def plot_ws_wd_composite_profile(
    ...
    *,
    ...
    if tl_utc is None:
        raise ValueError(
            f"datetime must be a synoptic UTC time in {METRIC_START} .. {METRIC_END}, "
            f"got {datetime_utc!r}. Example: 2025-09-09 12:00:00"
        )
...
def plot_all_composite_profiles(
    df: pd.DataFrame,
    out_dir: Path,
    *,
    tz: str = "utc",
    ...
    """为 campaign 内全部 synoptic 时次各生成一张四站 composite 廓线图。"""
    save_paths: list[Path] = []
    for dt in cfg.METRIC_DATETIMES:
        dt_str = dt.strftime("%Y-%m-%d %H:%M:%S")
        save_paths.append(
            plot_ws_wd_composite_profile(
                df,
                dt_str,
                out_dir,
                tz=tz,
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Plot single-time WS/WD composite profiles (3-station pooled, 1×2 panels).",
    )
    p.add_argument(
        "--datetime",
        help='UTC synoptic hour string, e.g. "2025-09-09 12:00:00" (must exist in merged CSV).',
    )
    p.add_argument(
        "--all-times",
        action="store_true",
        help="Plot composite profiles for every synoptic time in campaign_config.METRIC_DATETIMES.",
    )
    ...
    p.add_argument(
        ...
        help=(
            "Height selection cutoff (m): keep Height<=zmax for aggregation, "
            f"then set ylim to the same. Default: {int(DEFAULT_ZMAX)}."
        ),
    )
...
def main() -> None:
    args = _parse_args()
    if not args.all_times and not args.datetime:
        raise SystemExit("Provide --datetime or use --all-times.")
    configure_matplotlib_style()
    df = quality_control(load_and_preprocess(DATA_PATH))
    if args.all_times:
        save_paths = plot_all_composite_profiles(
            df,
            args.out_dir,
            tz=args.tz,
            zmax=args.zmax,
        ...
        print(f"Total: {len(save_paths)} figures")
        return
