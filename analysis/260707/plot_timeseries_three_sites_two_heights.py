...
# Default data window: 2025-09-07 00:00 – 2025-09-08 23:00 UTC
# (= 2025-09-07 08:00 – 2025-09-09 07:00 UTC+8)
DEFAULT_T_START_UTC = "2025-09-07 00:00:00"
DEFAULT_T_END_UTC = "2025-09-08 23:00:00"

# X-axis display limits in UTC+8 (padding around the data window)
DEFAULT_XLIM_LEFT_LST = "2025-09-07 06:00:00"
DEFAULT_XLIM_RIGHT_LST = "2025-09-09 12:00:00"
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=REPO_ROOT / "results/timeseries_3x2" / cfg.RESULTS_TAG,
        help="Output directory.",
    )
    ap.add_argument(
        "--t-start",
        type=str,
        default=DEFAULT_T_START_UTC,
        help=f"UTC start datetime (inclusive). Default: {DEFAULT_T_START_UTC}",
    )
    ap.add_argument(
        "--t-end",
        type=str,
        default=DEFAULT_T_END_UTC,
        help=f"UTC end datetime (inclusive). Default: {DEFAULT_T_END_UTC}",
    )
    ap.add_argument(
        "--3h-rolling",
        dest="rolling_3h",
    ...
    t_start = pd.Timestamp(args.t_start, tz="UTC")
    t_end = pd.Timestamp(args.t_end, tz="UTC")
    if t_end < t_start:
        raise ValueError(f"--t-end ({t_end}) is before --t-start ({t_start})")
    ...
    if df.empty:
        raise ValueError(f"No rows in [{t_start}, {t_end}] from {csv_path}")

    tz_mode = str(args.tz).lower()
    rolling_3h = bool(args.rolling_3h)

    available_times = pd.DatetimeIndex(sorted(df["datetime"].dropna().unique()))
    print(
        f"[Info] Plot window UTC: {t_start} → {t_end}  |  "
        f"available slots in CSV: {len(available_times)} / "
        f"{int((t_end - t_start) / pd.Timedelta(hours=1)) + 1} hourly"
    )
    ...
    # Fixed axis bounds in UTC+8; convert if plotting in UTC.
    x0_lst = pd.Timestamp(DEFAULT_XLIM_LEFT_LST, tz=timezone(timedelta(hours=8)))
    x1_lst = pd.Timestamp(DEFAULT_XLIM_RIGHT_LST, tz=timezone(timedelta(hours=8)))
    if tz_mode == "lst":
        x0, x1 = x0_lst, x1_lst
    else:
        x0, x1 = x0_lst.tz_convert(timezone.utc), x1_lst.tz_convert(timezone.utc)
    ...
    for i, site in enumerate(sites):
        ...
        for j, h_req in enumerate(height_reqs):
            ...
            # Metrics over the selected continuous window
            ...
            ax.set_xlim(x0, x1)
            ax.xaxis.set_major_locator(mdates.HourLocator(byhour=(0, 6, 12, 18), tz=tzinfo))
