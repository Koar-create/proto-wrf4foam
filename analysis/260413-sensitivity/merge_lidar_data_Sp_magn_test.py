...
output_file = data_dir_opt / "merged_lidar_simulation_final_Sp_magn_test.csv"
...
# CFD 文件列表 (Reference vs Sensitivity 1 vs Sensitivity 2 vs Sensitivity 3)
cfd_files_ref = [
    "CFD_lidar_simulation_20250903_1200_two_boundaries_as_outlet.csv",
    "CFD_lidar_simulation_20250903_1500_two_boundaries_as_outlet.csv",
    "CFD_lidar_simulation_20250903_1800_two_boundaries_as_outlet.csv"
]
cfd_files_sen = [
    "CFD_lidar_simulation_20250903_1200_two_boundaries_as_outlet-fvOpt_sensitivity_run.csv",
    "CFD_lidar_simulation_20250903_1500_two_boundaries_as_outlet-fvOpt_sensitivity_run.csv",
    "CFD_lidar_simulation_20250903_1800_two_boundaries_as_outlet-fvOpt_sensitivity_run.csv"
]
cfd_files_sen2 = [
    "CFD_lidar_simulation_20250903_1200_two_boundaries_as_outlet-fvOpt_sensitivity_run-002.csv",
    "CFD_lidar_simulation_20250903_1500_two_boundaries_as_outlet-fvOpt_sensitivity_run-002.csv",
    "CFD_lidar_simulation_20250903_1800_two_boundaries_as_outlet-fvOpt_sensitivity_run-002.csv"
]
cfd_files_sen3 = [
    "CFD_lidar_simulation_20250903_1200_two_boundaries_as_outlet-fvOpt_sensitivity_run-005.csv",
    "CFD_lidar_simulation_20250903_1500_two_boundaries_as_outlet-fvOpt_sensitivity_run-005.csv",
    "CFD_lidar_simulation_20250903_1800_two_boundaries_as_outlet-fvOpt_sensitivity_run-005.csv"
]

def load_and_preprocess():
    print("Step 1: Loading WRF and Lidar data...")
    df_wrf = pd.read_csv(os.path.join(data_dir / "wrf" / "WRF_lidar_simulation_1h-rolling.csv"))
    df_lidar = pd.read_csv(os.path.join(data_dir / "lidar" / "lidar_1h-rolling.csv"))
    ...
    def process_cfd_files(file_list, folder):
        cfd_list = []
        for f in file_list:
            tmp = pd.read_csv(os.path.join(data_dir, "cfd", folder, f))
    ...
    df_cfd_sen3 = process_cfd_files(cfd_files_sen3,"sensitivity")
    print(f"CFD Ref: {df_cfd_ref.shape}, Sen (-0.01): {df_cfd_sen.shape}, Sen (-0.02): {df_cfd_sen2.shape}, Sen (-0.05): {df_cfd_sen3.shape}")

    # 为各表添加 "高度层索引"，确保逐层对齐
    print("Step 3: Aligning vertical layers by index...")
    for df in [df_wrf, df_lidar, df_cfd_ref, df_cfd_sen, df_cfd_sen2, df_cfd_sen3]:
    ...
    # 4. 合并 CFD Sensitivity 2 (Sp = -0.02)
    ...
    # 5. 合并 CFD Sensitivity 3 (Sp = -0.05)
    df_cfd_sen3_sub = df_cfd_sen3[['datetime', 'obtid', 'layer_idx', 'U_cfd', 'V_cfd', 'W_cfd']].rename(columns={
        'U_cfd': 'u_cfd_sen3', 'V_cfd': 'v_cfd_sen3', 'W_cfd': 'w_cfd_sen3'
    })
    merged = pd.merge(merged, df_cfd_sen3_sub, on=['datetime', 'obtid', 'layer_idx'], how='inner')
    ...
    final_columns = [
        ...
        'u_cfd_sen3', 'v_cfd_sen3', 'w_cfd_sen3'
    ]
