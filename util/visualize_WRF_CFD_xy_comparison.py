...
Usage
-----
    python visualize_WRF_CFD_xy_comparison.py /path/to/CFD_control_run_directory --height 100

Example
-------
    python visualize_WRF_CFD_xy_comparison.py \
...
def draw_wrf_panel(ax, data: dict, vmax=None, label='(a) WRF'):
    ...
    ax.quiverkey(qv, X=0.87, Y=1.02, U=4, label='4 m/s',
...
def draw_cfd_panel(ax, data: dict, vmax=None, label='(b) CFD'):
    ...
    if 'Sensitivity' not in label:
        ...
        ax.quiverkey(qv, X=0.87, Y=0.975, U=1, label='1 m/s',
                    labelpos='E', coordinates='axes',
                    fontproperties={'family': 'serif', 'size': 10, 'weight': 'bold'})
