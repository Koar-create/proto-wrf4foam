...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Export a z-normal slice from OpenFOAM to CSV at a given time step."
    )
    parser.add_argument("foam", help="Path to the .foam case file")
    parser.add_argument("--z", type=float, required=True, help="Z coordinate of the slice (m)")
    parser.add_argument(
        "--time",
        type=float,
        required=True,
...
def main():
    ...
    out_csv = os.path.join(out_dir, f"{z_str}m_t{time_str}.csv")
