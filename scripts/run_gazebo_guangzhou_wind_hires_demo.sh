#!/usr/bin/env bash
# 高精度 LUT 视觉 demo：gazebo_wind_plugin/worlds/guangzhou_wind_hires_demo.world
# 须在仓库根目录执行：cd /path/to/WRF-OpenFOAM-Coupling && ./scripts/run_gazebo_guangzhou_wind_hires_demo.sh <子命令>
set -euo pipefail

WORLD_REL="gazebo_wind_plugin/worlds/guangzhou_wind_hires_demo.world"
...
用法（在 WRF-OpenFOAM-Coupling 根目录）:
  ./scripts/run_gazebo_guangzhou_wind_hires_demo.sh <子命令>
...
等价手工命令参考（与历史会话 fea3150c 验证一致）:
  export GAZEBO_PLUGIN_PATH="$PWD/gazebo_wind_plugin/build:${GAZEBO_PLUGIN_PATH:-}"
  export GAZEBO_MODEL_PATH="$PWD/gazebo_wind_plugin/models:/usr/share/gazebo-11/models:${GAZEBO_MODEL_PATH:-}"
  export GAZEBO_MODEL_DATABASE_URI=""
  gzserver gazebo_wind_plugin/worlds/guangzhou_wind_hires_demo.world --verbose
  gazebo gazebo_wind_plugin/worlds/guangzhou_wind_hires_demo.world --verbose
